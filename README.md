...
2. SRF Jobs 우상단 설정에서 `텔레그램 봇 토큰`을 저장합니다.
3. 각 사용자는 텔레그램에서 해당 봇에게 `/start`를 보냅니다.
4. SRF Jobs 설정에서 `chat_id 가져오기`를 누르거나 직접 chat_id를 입력합니다.
5. 알림 검색어, 구분, 출처, 태그를 저장합니다.
6. 새 공고가 저장될 때 개인 필터와 맞으면 텔레그램으로 한 번만 알림을 보냅니다.
