...
- 원문 링크와 지원 링크 연결
- 직접 공고 추가
- KOFIA 공개 채용안내 상세 페이지까지 확인해 1시간마다 자동 수집
- 공유 사람인 access-key가 설정되어 있으면 사람인 공식 Job Search API도 1시간마다 자동 수집
- 슈퍼루키 공개 채용공고도 1시간마다 자동 수집
- 개인별 텔레그램 알림 필터와 테스트 발송

## 텔레그램 알림 설정

텔레그램 알림은 단체방이 아니라 개인별로 발송합니다.

1. 운영자가 Telegram의 BotFather에서 봇을 만들고 bot token을 받습니다.
2. Render 환경변수 `TELEGRAM_BOT_TOKEN`에 bot token을 저장합니다. 로컬 개발에서는 `data/config.json`에만 저장합니다.
3. 각 사용자는 SRF Jobs 설정에서 `연결 코드 만들기`를 누릅니다.
4. 텔레그램에서 해당 봇에게 화면에 나온 연결 코드를 보냅니다.
5. SRF Jobs 설정에서 `연결 확인`을 누릅니다.
6. 알림 받을 태그를 버튼으로 선택하고 저장합니다.
7. 새 공고가 저장될 때 개인 태그 필터와 맞으면 텔레그램으로 한 번만 알림을 보냅니다.

봇 토큰, 사람인 키, 공유 비밀번호 같은 서버 민감 설정은 웹 설정창에서 바꾸지 않습니다.
